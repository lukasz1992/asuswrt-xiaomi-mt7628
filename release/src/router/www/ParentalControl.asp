...
var OTHERS_SUPPORT = 0;
var OTHERS_ENABLE = "0";
var OTHERS_DEVICENAME = "Others";
var OTHERS_MAC = "";
var OTHERS_DAYTIME = "<";
...
function initial(){
	show_menu();
	if(based_modelid == "GT-AC5300" || based_modelid == "GT-AC9600" || based_modelid == "RT-AC1200" || based_modelid == "RT-AC1200GU" || based_modelid == "RT-4C"){
		$("#nat_desc").hide();
	}
...
function gen_mainTable(){
	...
	if(MULTIFILTER_MAC == "")
	{
		MULTIFILTER_ENABLE = OTHERS_ENABLE;
		MULTIFILTER_MAC = OTHERS_MAC;
		MULTIFILTER_DEVICENAME = OTHERS_DEVICENAME;
		MULTIFILTER_MACFILTER_DAYTIME = OTHERS_DAYTIME;
	}
	else if(MULTIFILTER_MAC_row[MULTIFILTER_MAC_row.length-1] != "")
	{
		MULTIFILTER_ENABLE += ">" + OTHERS_ENABLE;
		MULTIFILTER_MAC += ">" + OTHERS_MAC;
		MULTIFILTER_DEVICENAME += ">" + OTHERS_DEVICENAME;
	...
	{
		OTHERS_ENABLE = MULTIFILTER_ENABLE_row[MULTIFILTER_ENABLE_row.length-1];
		OTHERS_DAYTIME = MULTIFILTER_MACFILTER_DAYTIME_row[MULTIFILTER_MACFILTER_DAYTIME_row.length-1];
	}
	...
	if(MULTIFILTER_DEVICENAME == "" && MULTIFILTER_MAC == "")
		...
		for(var i=0; i<MULTIFILTER_DEVICENAME_row.length; i++){
			...
			if(OTHERS_SUPPORT && MULTIFILTER_MAC_row[i] == "")
				code +='<td> </td>';
			else
...
function addRow_main(upper){
	...
	{
		MULTIFILTER_ENABLE = MULTIFILTER_ENABLE.substring(0,MULTIFILTER_ENABLE.lastIndexOf(">"));
		MULTIFILTER_DEVICENAME = MULTIFILTER_DEVICENAME.substring(0,MULTIFILTER_DEVICENAME.lastIndexOf(">"));
		MULTIFILTER_MAC = MULTIFILTER_MAC.substring(0,MULTIFILTER_MAC.lastIndexOf(">"));
		MULTIFILTER_MACFILTER_DAYTIME = MULTIFILTER_MACFILTER_DAYTIME.substring(0,MULTIFILTER_MACFILTER_DAYTIME.lastIndexOf(">"));
	}
    }
...
function genEnableArray_main(j, obj){
        ...
        for(i=0; i<MULTIFILTER_ENABLE_row.length; i++){
                ...
                if(i<MULTIFILTER_ENABLE_row.length-1)
                        MULTIFILTER_ENABLE += ">";
		if(i == MULTIFILTER_ENABLE_row.length - 1)
			OTHERS_ENABLE = MULTIFILTER_ENABLE_row[i];
        }       
}
