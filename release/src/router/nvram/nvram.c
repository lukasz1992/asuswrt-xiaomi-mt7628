...
usage(void)
{
	fprintf(stderr, "usage: nvram [get name] [set name=value] [unset name] [erase] [show] [save file] [restore file] [fb_save file]\n");
	exit(0);
}
	...
	for (; *argv; argv++) {
		...
		else if (!strncmp(*argv, "fb_save", 7))
		{
			if (*++argv)
			{
				...
				if(fp)
				{
					fwrite(tmpbuf, 1, nvram_space, fp);
					fclose(fp);
				}
#endif
