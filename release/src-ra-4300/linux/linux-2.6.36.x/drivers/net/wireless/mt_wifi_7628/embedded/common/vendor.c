		...
		)
{
	...
	UCHAR mtk_vht_cap[] = {
		...
		0x2A, 0xFF, 0x92, 0x04, 0x2A, 0xFF, 0x92, 0x04 /*upported MCS and Nss*/
	};
	UCHAR mtk_vht_op[] = {
		0xC0, /*EID*/
		0x05, /*LEN*/
	...
	if (pAd->chipCap.g_band_256_qam && WMODE_CAP(wdev->PhyMode, WMODE_GN))
		ra_ie.cap0 |= RALINK_256QAM_CAP;
	MakeOutgoingFrame(frame_buffer,
			&ra_ie_len, (ra_ie.ie_hdr.len + 2), &ra_ie, END_OF_ARGS);
