			...
			if (pCredential->EncrType & (WSC_ENCRTYPE_TKIP | WSC_ENCRTYPE_AES))
			{
				...
				if (pCredential->KeyLength >= 8 && pCredential->KeyLength <= 64)
				{
					...
					if (CurOpMode == AP_MODE)
					{
						...
						RTMPMoveMemory(pAd->ApCfg.MBSSID[CurApIdx].WPAKeyString,
									pCredential->Key, pWscControl->WpaPskLen);
	...
	if (pWscControl->WscPBCBssCount == 1)
	{
		...
		if (CurOpMode == AP_MODE)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("will connect %s (%02X:%02X:%02X:%02X:%02X:%02X) on %d\n",
				pWscControl->WscSsid.Ssid, PRINT_MAC(pWscControl->WscBssid),
	...
	IN	RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("WscPBCSessionOverlapClear : WscPBCStaProbeCount = %d\n",
				pWscStaPbcProbeInfo->WscPBCStaProbeCount));
}
