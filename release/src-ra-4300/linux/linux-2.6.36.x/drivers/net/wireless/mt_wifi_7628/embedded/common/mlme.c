...
#if (defined(MWDS) || defined(STA_FORCE_ROAM_SUPPORT)) || defined(WAPP_SUPPORT)
...
VOID MlmeHandler(RTMP_ADAPTER *pAd) 
{
		...
		) 
	{
		...
		process_cnt++;
		if ((!in_interrupt()) && (process_cnt >= (MAX_LEN_OF_MLME_QUEUE >> 1))) {
			process_cnt = 0;
			OS_SCHEDULE();
		}
			...
			switch (Elem->Machine) 
			{
				...
				case MIX_MODE_STATE_MACHINE:
					StateMachinePerformAction(pAd,
							&pAd->MixModeCtrl.MixModeStatMachine, Elem,
