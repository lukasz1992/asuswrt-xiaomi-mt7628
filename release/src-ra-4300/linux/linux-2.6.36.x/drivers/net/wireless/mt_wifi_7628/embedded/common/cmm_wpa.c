				...
				*/
					...
					(peerKeyInfo.KeyMic == 1))
				{
					...
					else if (peerKeyInfo.Secure == 1 && peerKeyInfo.KeyType == GROUPKEY)
					{
						...
						PeerGroupMsg2Action(pAd, pEntry, &Elem->Msg[hdr_len], (Elem->MsgLen - hdr_len));
					}
				}
#ifdef CONFIG_AP_SUPPORT
    ...
    IN ULONG			TimeInterval)
{
		...
		) && (pEntry->PMKID_CacheIdx != ENTRY_NOT_FOUND))
	{
		...
		if (pEntry->AuthMode == Ndis802_11AuthModeWPA3PSK) {
			...
			if (!sae_get_pmk_cache(&pAd->SaeCfg, pMbss->wdev.bssid,
				pEntry->Addr, digest, NULL)) {
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
	...
	if ((pEntry->AllowInsPTK == TRUE) && bWPA2) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(%d)-%d: update CCMP_BC_PN to %llu\n",
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (pApCliEntry) {
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,
					...
					__FUNCTION__, pApCliEntry->MlmeAux.IsSupportSHA256KeyDerivation, pApCliEntry->MlmeAux.RsnCap.field.MFPR));
...
IN MAC_TABLE_ENTRY * pEntry)
{
	...
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			/* Only support WPAPSK or WPA2PSK for AP-Client mode */
#ifdef WPA_SUPPLICANT_SUPPORT
			UCHAR temp = pAd->ApCfg.ApCliTab[apcliIfidx].wpa_supplicant_info.WpaSupplicantUP;

			if (temp != WPA_SUPPLICANT_DISABLE) {
				if (AuthMode < Ndis802_11AuthModeWPA)
					return;
			} else
#endif /* WPA_SUPPLICANT_SUPPORT */
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("==> RTMPMakeRSNIE(STA)\n"));
