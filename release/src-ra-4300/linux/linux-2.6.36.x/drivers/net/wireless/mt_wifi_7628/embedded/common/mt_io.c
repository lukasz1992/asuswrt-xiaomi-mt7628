...
void mt_mac_GPIO_range_mapping(struct _RTMP_ADAPTER *pAd, UINT32 *mac_addr)
{
	UINT32 mac_addr_hif = *mac_addr;
	UINT32 pcie_addr;

	if (mac_addr_hif == 0xb0000060)
		pcie_addr = 0x0;
	else if (mac_addr_hif == 0xb0000064)
		pcie_addr = 0x4;
	else if (mac_addr_hif == 0xb0000600)
		pcie_addr = 0x5a0;
	else if (mac_addr_hif == 0xb0000604)
		pcie_addr = 0x5a4;
	else if (mac_addr_hif == 0xb0000620)
		pcie_addr = 0x5c0;
	else if (mac_addr_hif == 0xb0000624)
		pcie_addr = 0x5c4;
	else {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s(): Invalid Physical addr\n", __func__));
		pcie_addr = 0;
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): mac_addr = %x\n", __func__, *mac_addr));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s(): The Physical addr = %x, the Pcie addr = %x\n",
		__func__, mac_addr_hif, pcie_addr));

	*mac_addr = pcie_addr - 0x2fffa0;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s() NEW: mac_addr = %x\n", __func__, *mac_addr));
}
