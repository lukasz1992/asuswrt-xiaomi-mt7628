	...
	IN PRRM_MLME_BCN_REQ_INFO pMlmeBcnReq)
{
	...
	*/
	...
	if (pMlmeBcnReq->BcnReqCapFlag.field.ReportCondition == TRUE)
	{
		...
		TotalLen += (2 + 2); /* report condition sub field. */
	}
	...
	if (BcnReq.MeasureMode == RRM_BCN_REQ_MODE_BCNTAB)
		ReportDetail = 0;
		/*force report detail to 0 when it is table mode*/
...
#define MIN(_x, _y) ((_x) > (_y) ? (_x) : (_y))
	...
	for (loop = 0; loop < pAd->ScanTab.BssNr; loop++)
	{
		...
		if (BssMatch)
		{
			...
			BssidInfo.field.Security = RRM_CheckBssAndStaSecurityMatch(pAd, pEntry, pBssEntry); /* rrm to do. */
