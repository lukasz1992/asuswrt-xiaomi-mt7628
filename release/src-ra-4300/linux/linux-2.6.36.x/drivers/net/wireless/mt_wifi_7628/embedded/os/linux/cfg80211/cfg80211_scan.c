	...
	VOID						*pAdOrg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("---> CFG80211_MLME Disconnect in Scaning, ORI ==> %d\n", 
	...
	ScanNextChannel(pAd, OPMODE_STA, &(pAd->StaCfg[0].wdev));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<--- CFG80211_MLME Disconnect in Scan END, ORI ==> %d\n", 
									pAd->Mlme.CntlMachine.CurrState)); 
	...
	IN INT32						RSSI)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("80211> Network is down!\n"));
		return;
	} 

	if (!pCfg80211_CB)
    {
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("80211> pCfg80211_CB is invalid!\n"));
            return;
    }
