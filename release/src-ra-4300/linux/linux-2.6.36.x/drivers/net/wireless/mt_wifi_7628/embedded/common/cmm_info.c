...
INT	Set_SendBMToAir_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pAd->BSendBMToAir)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("Set_SendBMToAir_Proc Enable B/M Pkts to air\n"));
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("Set_SendBMToAir_Proc Disable B/M Pkts to air\n"));
	...
	IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	...
	if (msg == NULL) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("RTMPIoctlGetDriverInfo - msg memory alloc fail.\n"));
		return;
	}
	...
	wrq->u.data.length = strlen(msg);
	if (copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length))
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: copy_to_user() fail\n", __func__));
	os_free_mem(NULL, (PUCHAR)msg);
...
INT Set_DynamicRaInterval(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s()::bDynamicRaInterval=%d\n",
...
INT Set_SwiftTrainThrd(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    ...
    MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s()::SwiftTrainThrd=%d\n",
...
#ifdef MWDS
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("MWDSCap"));
#endif /* MWDS */
		...
		if (IS_ENTRY_APCLI(pEntry)) {
			if (pEntry->func_tb_idx < MAX_APCLI_NUM) {
				if (pAd->ApCfg.ApCliTab[pEntry->func_tb_idx].MlmeAux.bSupportMWDS)
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("YES"));
				else
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("NO"));
			}
		} else {
#endif
	...
	if (ret == NDIS_STATUS_SUCCESS) {
		...
		while ((thisChar = strsep((char **)&arg, "-")) != NULL) {
			...
			case 4: /* mac_addr. */
				...
				if (strlen(thisChar) == 17)	{
					RTMP_STRING *value;
					for (Loop = 0, value = rstrtok(thisChar, ":"); value;
						value = rstrtok(NULL, ":")) {
...
INT Show_sta_tr_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (idx = 0; idx < MAX_LEN_OF_TR_TABLE; idx++)
	{
		...
		if (IS_VALID_ENTRY(tr_entry))
			dump_tr_entry(pAd, idx, __func__, __LINE__);
	}
#endif /* DBG */
	return TRUE;
}
				...
				&& (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED))
 				{
					...
					if (pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.bSupportMWDS)
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("MWDSCap : YES\n"));
					else
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("MWDSCap : NO\n"));
#endif /* MWDS */
 				}
 		}
