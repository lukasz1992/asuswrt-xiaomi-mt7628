...
VOID MlmeRAInit(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	ra_swift_train_up_table_reset(pEntry);/* RAA V3 */
	...
	IN ULONG			TxTotalCnt)
{
	...
	)
	{
		...
		if (raLogType==RAL_QUICK_DRS)
			tp = (100-TxErrorRatio)*TxTotalCnt*pAd->ra_interval/(100*pAd->ra_fast_interval);
			...
			tp = (100-TxErrorRatio)*TxTotalCnt*pAd->ra_interval/(100*(pAd->ra_interval-pAd->ra_fast_interval));
