				    ...
				    (pRxBlk->wcid == RESERVED_WCID))
				{
					...
					if (IS_ENTRY_APCLI(pEntry)) {
						...
						if (pApCliEntry) {
							...
							if ((NdisCmpMemory(pApCliEntry->wdev.if_addr, pHeader->Addr1,
								MAC_ADDR_LEN))
...
#ifdef MWDS
	FRAME_CONTROL *FC = (FRAME_CONTROL *)(&(pRxBlk->pHeader->FC));
#endif
...
#ifdef CONFIG_HOTSPOT
		if (pEntry && pEntry->pMbss && pEntry->pMbss->HotSpotCtrl.HotSpotEnable) {
	...
	if (VALID_WCID(pRxBlk->wcid))
	{
		...
		*/
		...
		if (pEntry) {
			pRxBlk->wcid = pEntry->wcid;
#ifdef APCLI_SUPPORT
			/* add by bo.jiao for R1 Ping issue */
			if (IS_ENTRY_APCLI(pEntry)) {
				PAPCLI_STRUCT pApCliEntry = NULL;
