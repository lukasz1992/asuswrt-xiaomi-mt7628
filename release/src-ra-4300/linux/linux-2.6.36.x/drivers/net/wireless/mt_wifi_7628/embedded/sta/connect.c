...
//---Add by shiang
	...
	if (wdev->PortSecured == WPA_802_1X_PORT_SECURED) /*For security NONE & WEP case*/
		wapp_send_apcli_association_change(WAPP_APCLI_ASSOCIATED, pAd, &pAd->StaCfg);
#endif /*WAPP_SUPPORT*/
...
INT LinkDown_Infra(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, BOOLEAN ReqByAP)
{
	...
	*/
	...
	map_a4_peer_disable(pAd, &pAd->MacTab.Content[pAd->StaCfg.MacTabWCID], FALSE);
#endif
#if defined(MAP_SUPPORT) && defined(WAPP_SUPPORT)
	wapp_send_apcli_association_change(WAPP_APCLI_DISASSOCIATED, pAd, &pAd->StaCfg);
#endif /*WAPP_SUPPORT*/
