	...
	for (i = 0; i < WDEV_NUM_MAX; i++) {
		if (pAd->wdev_list[i] != NULL) {
			...
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("7628 (%s) interface:%s, MacAddr = %02x:%02x:%02x:%02x:%02x:%02x\n",
				__func__, dev_info->ifname, PRINT_MAC(dev_info->mac_addr)));
	...
	UINT8 Action)
{
	...
	case OFFCHANNEL_INFO_RSP:
		...
		Rsp.data.channel_data.actual_measured_time =
			pAd->ScanCtrl.ScanTimeActualDiff == 0 ? 1 : pAd->ScanCtrl.ScanTimeActualDiff;
