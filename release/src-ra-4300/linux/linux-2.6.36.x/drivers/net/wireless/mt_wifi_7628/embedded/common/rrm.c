...
INT Set_BeaconReq_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (ArgIdx < 7 || ArgIdx > 11)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s: invalid args (%d).\n", __func__, ArgIdx));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("eg: iwpriv ra0 set BcnReq=<Aid>-<Duration>-<RegulatoryClass>-"
				 "<BSSID>-<SSID>-<MeasureCh>-<MeasureMode>-<ChRegClass>-<ChReptList>\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	if (DialogToken != 0) {
		...
		if (pFramePtr[2] != 0) {
			MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("DialogToken is zero, MeasurementToken is non-zero, check MeasurementToken!\n"));
