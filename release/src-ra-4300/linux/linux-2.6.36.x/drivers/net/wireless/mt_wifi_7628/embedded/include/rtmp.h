...
/* ==================================================== */
	...
	/* to record the each TX rate's quality. 0 is best, the bigger the worse. */
	UINT8 TxQuality[MAX_TX_RATE_INDEX + 1];/* RAA V3 */
	UINT8 SwiftTrainUpRate[SWIFT_TRAIN_UP_RECORD];/* RAA V3 */
	CHAR SwiftTrainUpRSSI[SWIFT_TRAIN_UP_RECORD];/* RAA V3 */
	BOOLEAN fLastSecAccordingRSSI;
	UCHAR LastSecTxRateChangeAction;	/* 0: no change, 1:rate UP, 2:rate down */
	...
	BOOLEAN bSwiftTrain; /* Configure to Swift Train down */
	UINT32 SwiftTrainThrd;
	UINT32 rxv2_cyc3[10]; /* store CN */
	UCHAR PER[MAX_TX_RATE_INDEX + 1];
...
/***********************************************************/
	...
	UCHAR MaxTxPwr; /* announced in beacon to decide peer's capality */
...
INT Set_Fixed_Rate_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);/* RAA V2 */
