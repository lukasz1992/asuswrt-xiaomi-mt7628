	...
	*/
	...
	if (pMbss->PmfCfg.Desired_MFPR != pMbss->PmfCfg.MFPR &&
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[PMF]%s:: PmfCfg Desired MFPR error\n", __func__));
	}
#endif
#if defined(DOT11_SAE_SUPPORT) || defined(CONFIG_OWE_SUPPORT)
	if (pMbss->PmfCfg.Desired_MFPC != pMbss->PmfCfg.MFPC &&
		...
		wdev->AuthMode == Ndis802_11AuthModeOWE)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("[PMF]%s:: PmfCfg Desired MFPC error\n", __func__));
	}
#endif
...
INT ap_security_init(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, INT idx)
{
	...
	if (wdev->WepStatus == Ndis802_11Encryption2Enabled || wdev->WepStatus == Ndis802_11Encryption3Enabled ||
...
VOID APStopBssOnly(RTMP_ADAPTER *pAd, BSS_STRUCT *pMbss)
{
	...
	if (!wdev_bss) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Error!!! wdev_map is null !!!\n"));
		return;
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! APStop !!!\n"));
...

{
	...
	if ((needFallBack == FALSE)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rollback the bandwidth setting to support 20/40Mhz\n"));
	}
