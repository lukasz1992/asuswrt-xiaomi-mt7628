...
INT Set_ApOffChanScan_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
#endif
...
static INT Set_RoamingEnhance_Enable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
	...
	IN INT cmd)
{
	...
	switch(cmd & 0x7FFF)
	{
			...
			case OID_802_11_CURRENT_CHANNEL_INFO:
			{
				...
				Rsp.actual_measured_time = pAd->ScanCtrl.ScanTimeActualDiff == 0 ? 1 : pAd->ScanCtrl.ScanTimeActualDiff;/*need change */
		...
		case OID_SET_PSK:
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: PSK = %s\n",
					__func__, PSK));
#ifdef APCLI_SUPPORT
				if (pObj->ioctl_if_type == INT_APCLI) {
					copy_from_user(pAd->ApCfg.ApCliTab[pObj->ioctl_if].PSK,
...
#endif
				{
					if ((pObj->ioctl_if_type == INT_MAIN || pObj->ioctl_if_type == INT_MBSSID)) {
						copy_from_user(pAd->ApCfg.MBSSID[pObj->ioctl_if].WPAKeyString,
							wrq->u.data.pointer, wrq->u.data.length);
...
#ifndef WAPP_SUPPORT /*WAPP_SUPPORT*/
	...
	case OID_802_11_WNM_BTM_REQ:
		{
			...
			if (pEntry) {
				if ((pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.AuthMode <= Ndis802_11AuthModeAutoSwitch)
					...
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("btm1\n"));
					Send_BTM_Req(pAd,
					     req_data->peer_mac_addr,
					     req_data->btm_req,
					     req_data->btm_req_len);
				} else {
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("btm2\n"));
	...
	case OID_802_11_WNM_NOTIFY_REQ:
		{
    		...
    		{
			} else if ((pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.AuthMode <= Ndis802_11AuthModeAutoSwitch)
					|| ((pEntry->WpaState == AS_PTKINITDONE) && (pEntry->GTKState == REKEY_ESTABLISHED)))
    		{
				printk("wnm1\n");
		...
		case OID_ANDLINK_STAINFO:
			...
			for (idx = 0; idx < MAX_LEN_OF_MAC_TABLE; idx++) {
				...
				if (pAd->ApCfg.bMACRepeaterEn && pEnt && IS_ENTRY_APCLI(pEnt)
					...
					pRepEnt = RTMPLookupRepeaterCliEntry(pAd, FALSE,
										pEnt->ReptCliAddr, TRUE, &isLinkValid);
					if (pRepEnt != NULL) {
						memcpy(sta_info.rept_item[sta_info.rept_sta_cnt].MacAddress,
								pRepEnt->OriginalAddress, MAC_ADDR_LEN);
						memcpy(sta_info.rept_item[sta_info.rept_sta_cnt].VMacAddr,
								pEnt->ReptCliAddr, MAC_ADDR_LEN);
				...
				if (pEnt->Sst == SST_ASSOC && pEnt->EntryType == ENTRY_CLIENT) {
...
#ifdef MAC_REPEATER_SUPPORT
					pRepEnt = RTMPLookupRepeaterCliEntry(pAd, TRUE,
										pEnt->Addr, TRUE, &isLinkValid);
					if (pRepEnt != NULL && pAd->ApCfg.bMACRepeaterEn)
						memcpy(sta_info.item[sta_info.sta_cnt].VMacAddr,
						pRepEnt->CurrentAddress, MAC_ADDR_LEN);
					else
#endif /* MAC_REPEATER_SUPPORT */
					...
					/* RSSI */
					sta_info.item[sta_info.sta_cnt].RSSI = RTMPAvgRssi(pAd, &pEnt->RssiSample);
		...
		case OID_ANDLINK_UPLINK:
			...
			for (idx = 0; idx < MAX_LEN_OF_MAC_TABLE; idx++) {
				...
				&& (!pEntry->bReptCli)
					...
					uplink_stas.RSSI = RTMPMaxRssi(pAd,
									pEntry->RssiSample.LastRssi[0],
									pEntry->RssiSample.LastRssi[1],
									pEntry->RssiSample.LastRssi[2]);
					uplink_stas.Noise = uplink_stas.RSSI - uplink_stas.SNR;
					/* TXRate */
					MCS = lastTxRate & 0x7F;
		...
		case OID_WSC_UUID:
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
						("7603 The UUID Hex string is:"));
					for (i = 0; i < UUID_LEN_HEX; i++)
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
						("%02x", (pWscControl->Wsc_Uuid_E[i])));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\n"));
					break;
				}
			...
			case OID_GET_BSS_INFO:
				{
					...
					NdisMoveMemory(bss_info->key, pAd->ApCfg.MBSSID[pObj->ioctl_if].PSK,
						bss_info.key_len);
#endif
...
INT	Set_AP_SSIDSingle_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (((pObj->ioctl_if < HW_BEACON_MAX_NUM)) && (strlen(arg) <= MAX_LEN_OF_SSID)) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			...
			pObj->ioctl_if, mbss->SsidLen, mbss->Ssid));
	} else
		success = FALSE;
	return success;
}
	...
	IN  RTMP_STRING *arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	UCHAR input = (UCHAR)simple_strtol(arg, 0, 10);
...
/*	RT_802_11_ACL			acl; */
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Entry #%02d: ", i+1));
...
static INT Set_RoamingEnhance_Enable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
		("(%s)::(enable = %d)\n",  __func__, pAd->ApCfg.bRoamingEnhance));
	return TRUE;
}
#endif/*ROAMING_ENHANCE_SUPPORT*/
#ifdef APCLI_SUPPORT
...
INT Set_led_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	status = (UCHAR) simple_strtol(arg, 0, 10);
	RTMPSetLED(pAd, status);
	...
	IN UINT32 ANQPRspLen)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s ANQPRspLen %d pGASCtrl->MMPDUSize %d\n",
	...
	if (bSMESHEn != mnt_enable)
        {
    		...
    		if (flush_all == TRUE)
    		{
    			...
    			pMntEntry = &pAd->MntTable[i];
    	                ...
    	                u4MAR1 |= MAR1_WRITE | MAR1_ACCESS_START_STATUS | ((MONITOR_MUAR_BASE_INDEX + i) << MAR1_ADDR_INDEX_OFFSET)
            ...
            {
    		HW_IO_READ32(pAd, RMAC_SMESH, &u4SMESH);			
    		u4SMESH |= SMESH_ADDR_EN | SMESH_RX_A2 | SMESH_RX_A1 
    				| SMESH_RX_DATA | SMESH_RX_MGT; 		
    		HW_IO_WRITE32(pAd, RMAC_SMESH, u4SMESH);
		HW_IO_READ32(pAd, RMAC_SMESH, &u4SMESH);
		pAd->MntEnable = 1;
           }
	...
	IN RTMP_STRING		*arg)
{
	...
	while ((this_char = strsep((char **)&arg, ";")) != NULL)
	{
		...
		if (strlen(this_char) != 3)  /* the acceptable format is like 0:1:1 with length 5 */
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, 
	...
	IN RTMP_STRING		*arg)
{
	...
	u4MAR1 |= MAR1_WRITE | MAR1_ACCESS_START_STATUS | ((MONITOR_MUAR_BASE_INDEX + mnt_idx) << MAR1_ADDR_INDEX_OFFSET)
			| (((UINT32)0 << MAR1_MAR_GROUP_OFFSET) & MAR1_MAR_GROUP_MASK);
	...
	u4MAR1 = MAR1_ACCESS_START_STATUS | ((MONITOR_MUAR_BASE_INDEX + mnt_idx) << MAR1_ADDR_INDEX_OFFSET);
	...
	IN RTMP_STRING		*arg)
{
	...
	for (i=0; i<MAX_NUM_OF_MONITOR_STA; i++)
	{
        ...
        if(pEntry->bValid)
        {
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Monitor STA[%d]\t", i));
