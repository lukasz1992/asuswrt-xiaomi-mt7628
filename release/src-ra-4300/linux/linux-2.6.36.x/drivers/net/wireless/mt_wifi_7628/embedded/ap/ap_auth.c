...
SendAuth:
		...
		&& (auth_info.auth_alg == AUTH_MODE_FT))
	{
				...
				if (result == MLME_SUCCESS)
				{
					...
					pEntry->Sst = SST_AUTH;
						...
						MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF,
							("%s - give up this AUTH pkt => Query R1KH from backbone(Wcid%d)\n",
		...
		if (pAd->ApCfg.BandSteering)
		{
			...
			if (bBndStrgCheck == FALSE) {
				APPeerAuthSimpleRspGenAndSend(pAd, pRcvHdr, auth_info.auth_alg, auth_info.auth_seq + 1, MLME_UNSPECIFY_FAIL);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AUTH - BndStrg check failed.\n"));
				/*return;*/
#ifdef WAPP_SUPPORT
				...
				|| (wdev->AuthMode == Ndis802_11AuthModeAutoSwitch)))
	{
		...
		if (pEntry)
		{
			...
			NStatus = MlmeAllocateMemory(pAd, &pOutBuffer);
			if (NStatus != NDIS_STATUS_SUCCESS) {
				/* if no memory, can't do anything */
#ifdef WAPP_SUPPORT
				wapp_auth_fail = MLME_NO_RESOURCE;
#endif /* WAPP_SUPPORT */
