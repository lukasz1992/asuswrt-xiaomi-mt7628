	...
	IN UCHAR MaxSupportedRate)
{
	...
	*/
	...
	if (pAd->chipCap.hif_type == HIF_MT
#ifdef DOT11R_FT_SUPPORT
		&& (!IS_FT_STA(pEntry) || (IS_FT_STA(pEntry) &&
		...
		case IE_WPA2:
			...
			if (NdisEqualMemory(MTK_OUI, eid_ptr->Octet, 3)) {
				if (MWDS_SUPPORT(eid_ptr->Octet[3])) {
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Peer supports MWDS\n"));
				}
				else
	...
	IN BOOLEAN isReassoc)
{
...
#if defined(DOT11_SAE_SUPPORT) || defined(CONFIG_OWE_SUPPORT)
#ifdef DOT11R_FT_SUPPORT
	NdisZeroMemory(&FtInfoBuf, sizeof(FtInfoBuf));
#endif
#endif
	...
	if ((pEntry->Sst == SST_ASSOC) && (isReassoc == FALSE)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("STA - %02x:%02x:%02x:%02x:%02x:%02x already associated! Ignore!\n",
...
SendAssocResponse:
	...
	if (IS_MAP_ENABLE(pAd)) {
		...
		/*
		* case: wifi interface is Ready in BH_BSS, but Don't connect to controller
		* map agent should be rejected, beacause map interface isn't ready.
		*/
		if ((IS_MAP_TURNKEY_ENABLE(pAd)) &&
		    (pEntry->DevPeerRole & BIT(MAP_ROLE_BACKHAUL_STA)) &&
		    ((wdev->MAPCfg.DevOwnRole & BIT(MAP_ROLE_BACKHAUL_BSS)) && !(is_controller_found(wdev)))) {
...
#ifdef MWDS
		if (tr_entry && (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED))
			MWDSAPPeerEnable(pAd, pEntry, FALSE);
#endif /* MWDS */
	}
