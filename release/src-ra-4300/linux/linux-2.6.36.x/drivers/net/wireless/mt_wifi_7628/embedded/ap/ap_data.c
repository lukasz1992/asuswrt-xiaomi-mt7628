		...
		if (pEntry && (pEntry->Sst == SST_ASSOC)) {
			...
			if (tr_entry && (tr_entry->PortSecured != WPA_802_1X_PORT_SECURED)) {
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					("sta port not secure, os should not send packets!!!\n"));
				return FALSE;
			}
	...
	if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bA4Frame)) {
		...
		if (pTxBlk->pMacEntry) {
			...
			if (IS_ENTRY_APCLI(pTxBlk->pMacEntry)) {
				COPY_MAC_ADDR(pHeader80211->Addr1, APCLI_ROOT_BSSID_GET(pAd, pTxBlk->Wcid));
	...
	if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bA4Frame)) {
		...
		if (pTxBlk->pMacEntry) {
			...
			if (IS_ENTRY_CLIENT(pTxBlk->pMacEntry)) {
				...
				COPY_MAC_ADDR(wifi_hdr->Addr2, pTxBlk->pMacEntry->wdev->bssid); /* from AP1 */
			}
...
#if defined(WDS_SUPPORT) || defined(CLIENT_WDS) || defined(A4_CONN) || defined(WH_EVENT_NOTIFIER)
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1)) {
		...
		if (pEntry && IS_ENTRY_A4(pEntry)) {
			...
			if (ProtoType == 0x0806) /* ETH_P_ARP */ {
				...
				hex_dump("ARPSenderIP", (PUCHAR)&ARPSenderIP, 4);
			}
/*
...
*/
			if (!bTAMatchSA) {
				...
				if (pMovedEntry
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
						("APRxPktAllow: AP found a entry(%02X:%02X:%02X:%02X:%02X:%02X)\n",
		...
		if (pEntry)
		{
#if defined(STATS_COUNT_SUPPORT) && defined(WDS_SUPPORT)
