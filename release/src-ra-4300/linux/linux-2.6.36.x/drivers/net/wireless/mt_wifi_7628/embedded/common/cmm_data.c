...
VOID ParseRxVPacket(RTMP_ADAPTER *pAd, UINT32 Type, RX_BLK *RxBlk, UCHAR *Data)
{
	...
	else if (Type == RMAC_RX_PKT_TYPE_RX_TXRXV)
	{
		...
		pEntry = MacTableLookup(pAd, Data);
		if (pEntry != NULL)
...
#ifdef CONFIG_DOT11V_WNM
				BSS_STRUCT *pMbss = NULL;
				if (wdev->wdev_type == WDEV_TYPE_AP && wdev->func_idx >= pAd->ApCfg.BssidNum) {
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,
						("wdev->wdev_type: %s(%d), wdev->func_idx(%d) > BssidNum(%d)\n",
						wdev->wdev_type == WDEV_TYPE_STA ? "STA" : "Other", wdev->wdev_type,
						wdev->func_idx, pAd->ApCfg.BssidNum));
					break;
				}
